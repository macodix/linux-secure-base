...
Dieses Dokument beschreibt die Härtung des Linux-Grundsystems: Maßstab und Geltung, das Schichtenmodell, Authentifizierung und Zugriff, Brute-Force-Schutz, Geheimnis-Trennung, Dienst-Isolation und die Härtungsprüfung.
...
Die Auswahl der Härtungsmaßnahmen folgt dem BSI-IT-Grundschutz. Die technische Konfiguration wird mit dem CIS-Benchmark für Ubuntu Server (Level 1) geprüft. Der Maßstab ist verbindlich. Abweichungen je Maßnahme werden begründet und in der Betriebsdokumentation festgehalten.

## 2. Defense-in-depth

Sicherheitskritischer Zugriff verlangt mehrere unabhängige Schichten. Der SSH-Zugriff verlangt Public-Key und TOTP als zweiten Faktor. Lokal gebundene Dienste sind von außen nicht direkt erreichbar.
...
Administrative Tätigkeiten laufen über den Wechsel zum Root-Konto per `su`. `sudo` gehört zur Ubuntu-Standardinstallation und bleibt installiert, weil der CIS-Benchmark es erwartet, wird aber nicht genutzt. Der Hauptbenutzer ist kein Mitglied administrativer Gruppen (insbesondere nicht der Gruppe `sudo`). Änderungen an der sudo-Konfiguration überwacht `auditd` (Kapitel 7).
...
Zugangsdaten, Token und Schlüssel liegen außerhalb des Repos in Dateien mit Mode 600 oder 640 und werden in der Konfiguration referenziert, nicht eingebettet. Die Geheimnis-Dateien liegen unter dem Konfig-Verzeichnis des jeweiligen Eigentümers.

Diese Stelle legt die maßgebliche Geheimnis-Datei-Regel fest. Alle Komponenten, die eine Geheimnis-Datei lesen oder prüfen, beziehen sich auf diese Regel und wiederholen sie nicht abweichend.

Geheimnis-Datei-Regel (maßgeblich):
...
Eine Komponente, die eine Geheimnis-Datei vor der Nutzung prüft, prüft fail-closed und lehnt bei jeder Abweichung ab. Geprüft wird:

- Keine Welt-Rechte: `o-rwx` ist erfüllt, andernfalls Ablehnung. Eine für die Welt lesbare Geheimnis-Datei wird nie genutzt.
