...
Gesichert werden im Grundzustand `/etc`, `/home`, `/var/log`, `/root` und `/var/backup`. Der Lauf arbeitet mit `--one-file-system`: Eingehängte Fremd-Dateisysteme unterhalb dieser Pfade (sshfs, davfs, NFS, Bind-Mounts) gehören nie zum Sicherungsumfang. Zwei Schutzvorkehrungen im Skript: Eine Zeitbegrenzung (12 h Sicherung, 2 h Aufbewahrungspflege) lässt einen hängenden Lauf als gewöhnlichen Fehler enden — die Fehler-Mail geht noch in derselben Nacht raus; eine Sperre (`flock` auf `/run/secure-base-backup.lock`) stellt sicher, dass höchstens ein Lauf zugleich läuft — ein von cron gestarteter zweiter Lauf bricht sofort ab und meldet sich ebenfalls per Fehler-Mail.
